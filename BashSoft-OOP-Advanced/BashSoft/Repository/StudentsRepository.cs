    ...
    public class StudentsRepository : IDatabase
    {
        ...
        private void ReadData(string fileName)
        {
            ...
            if (File.Exists(path))
            {
                ...
                //// my Regex:
                ////string pattern = @"([A-Z][a-zA-Z#\++]*_[A-Za-z]{3}_\d{4})\s+([A-Za-z]+\d{2}_\d{2,4})\s([\s0-9]+)";
                //string pattern = @"([A-Z][a-zA-Z#\++]*_[A-Za-z]+)\s+([A-Za-z]+\s+\d{2,4})";
                ...
                {
                    bool is1 = !string.IsNullOrEmpty(allInputLines[line]);
                    bool is2 = regex.IsMatch(allInputLines[line]); // тук не мачва!?
